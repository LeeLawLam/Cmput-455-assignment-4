...
class CommandInterface:
    ...
    # Simple evaluation of the current position from the point of view of self.to_play
    def evaluate(self):
        p1_score, p2_score = self.calculate_score()
        if self.to_play == 1:
            return p1_score - p2_score
    ...
    def _negamax(self, depth, alpha, beta, start_time, time_budget):
        ...
        # Move ordering: prefer moves closer to center
        cx = (self.width - 1) / 2.0
    ...
    def genmove(self, args):
        ...
        try:
            while True:
                # Check time before starting a deeper iteration
                if time.time() - start_time > time_budget:
                    break

                depth_best_move = best_move_so_far
                ...
                # Completed this depth: update global best
                best_move_so_far = depth_best_move
                best_value_so_far = depth_best_value
                depth += 1

        except TimeoutError:
            # Time ran out during search; fall back to best_move_so_far
            pass

        # Play the best move we found in time
        x, y = best_move_so_far
        self.make_move(x, y)
        print(x, y)
        return True
